...
module Gitosis
	def self.renderUrls(baseUrlStr, projectId, isReadOnly)
		...
		rendered = rendered + "<strong>" + t(isReadOnly ? :git_readonly_urls : :git_developer_urls, :count => baseUrlList.length == 1) + ": </strong><br/>"
	...
	def self.update_repositories(projects)
		...
		lockfile=File.new(File.join(Rails.root,"tmp",'redmine_gitosis_lock'),File::CREAT|File::RDONLY)
		retries=5
		loop do
		...
		# create tmp dir
		local_dir = File.join(Rails.root,"tmp","redmine_gitosis_#{Time.now.to_i}")
		...
		projects.select{|p| p.repository.is_a?(Repository::Git)}.each do |project|
			...
			# write config file
			conf = IniFile.new(nil, :filename => File.join(local_dir,'gitosis','gitosis.conf'))
			original = conf.clone
